...
A customer who creates an account on their first order (`want_account`, no earlier orders) gets `Customer.next_order_discount`. The following order takes 10% off merchandise (`account_offer_amount`), shown at checkout as “You're getting 10% off this order.” The flag is cleared inside `finalize_order`. That used amount is `Order.account_discount_amount` and is labelled **10% account offer** in admin. The first-order email says the next order gets 10% off. Promo codes stay separate and are not replaced by this offer.
...
**Cash on Delivery** is offered only when the shipping country is Bahrain. Any other destination shows Pay Online alone; a posted `cod` value for a non-Bahrain country is rejected. Choosing Cash on Delivery also requires the same signed add/subtract question used on `/register` (`captcha_token` / `captcha_answer`) before Place Order. Pay Online does not ask it.
